...
const modes = { continents: { name: "القارات", emoji: "🌍" }, animals: { name: "حيوانات مميزة", emoji: "🐾" }, mixed: { name: "مختلط", emoji: "🎲" } };
const continents = [
  { id: 'africa', name: 'أفريقيا', animal: 'أسد' },
  { id: 'asia', name: 'آسيا', animal: 'نمر' },
  { id: 'europe', name: 'أوروبا', animal: 'دب' },
  { id: 'america-north', name: 'أمريكا الشمالية', animal: 'بيسون' },
  { id: 'america-south', name: 'أمريكا الجنوبية', animal: 'لاما' },
  { id: 'australia', name: 'أستراليا', animal: 'كانغارو' },
  { id: 'antarctica', name: 'القارة القطبية', animal: 'بطريق' },
];
const emojiFor=(c)=>({ 'أفريقيا':'🦁', 'آسيا':'🐅', 'أوروبا':'🐻', 'أمريكا الشمالية':'🦬', 'أمريكا الجنوبية':'🦙', 'أستراليا':'🦘', 'القارة القطبية':'🐧' }[c]||'🌍');
function randFrom(arr,n){const c=[...arr];c.sort(()=>Math.random()-0.5);return n?c.slice(0,n):c[0];}

export default function ContinentsGame({ level="beginner", onComplete, onBack }={}){
  const router=useRouter(); const searchParams=useSearchParams();
  const [difficulty,setDifficulty]=useState(difficultyPresets[level]?level:"beginner");
  const [mode,setMode]=useState("continents"); const [started,setStarted]=useState(false);
  ...
  function makeQuestion(){ const actual=mode==='mixed'?randFrom(["continents","animals"]):mode; if(actual==='continents'){ const item=randFrom(continents); const wrong=randFrom(continents.filter(c=>c.id!==item.id),3).map(x=>x.name); setQuestion({ type:actual, text:`${emojiFor(item.name)} ما اسم هذه القارة؟`, correct:item.name, hint:"عدد القارات سبع" }); setOptions([item.name,...wrong].sort(()=>Math.random()-0.5)); } else { const item=randFrom(continents); const wrong=randFrom(continents.filter(c=>c.id!==item.id),3).map(x=>x.animal); setQuestion({ type:actual, text:`${emojiFor(item.name)} أي حيوان يميز ${item.name}؟`, correct:item.animal, hint:"تخيل بيئتها" }); setOptions([item.animal,...wrong].sort(()=>Math.random()-0.5)); }
  ...
  return (<Box sx={{maxWidth:900,mx:'auto',p:{xs:1.5,md:2}}}><Paper elevation={0} sx={{p:{xs:2,md:3},borderRadius:6,backgroundColor:'#fff',border:'3px solid #81c784'}}>
    <Box sx={{display:'flex',justifyContent:'space-between',alignItems:'center',mb:1}}>
      ...
      <Typography variant="h4" sx={{color:'#2e7d32',fontWeight:'bold'}}>قارات العالم</Typography>
    ...
    {!started? (<>
      ...
      <Box sx={{textAlign:'center',mb:1}}>
        <Typography variant="h6" sx={{color:'#1b5e20'}}>{question?.type==='continents'? 'اسم القارة' : 'حيوان مميز'}</Typography>
