...
export default function WordFromPictureGame({ level: initialLevel = 'beginner', onComplete, onBack }) {
  ...
  const [current, setCurrent] = useState(null); // {imageItem, options[], answer}
  const [questionStart, setQuestionStart] = useState(Date.now());

  const particleCanvasRef = useRef(null);
  const progressManager = useRef(new GameProgressionManager('word-from-picture'));
  ...
  const nextQuestion = () => {
    const answer = vocabItems[Math.floor(Math.random() * vocabItems.length)];
    const options = [answer];
    while (options.length < LEVEL_CONFIG[level].choices) {
      const w = vocabItems[Math.floor(Math.random() * vocabItems.length)];
      if (!options.find(o => o.id === w.id)) options.push(w);
    }
    options.sort(() => Math.random() - 0.5);
    setCurrent({ imageItem: answer, options, answer });
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mt: 4, position: 'relative' }}>
      ...
      <Paper elevation={8} sx={{ p: 4, borderRadius: 4, minWidth: 380, maxWidth: 700, border: `3px solid ${difficultyLevels[level].color}`, boxShadow: `0 8px 32px ${difficultyLevels[level].color}40`, background: 'linear-gradient(145deg, #ffffff, #f8f9fa)', position: 'relative', overflow: 'hidden' }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          ...
          <Typography variant="h4" align="center" sx={{ fontWeight: 'bold', color: difficultyLevels[level].color, flexGrow: 1 }}>قراءة الكلمات</Typography>
            ...
            <Zoom in={!showFeedback} timeout={500}>
              <Paper elevation={4} sx={{ p: 3, mb: 3, borderRadius: 3, backgroundColor: '#f8f9fa', border: '2px solid #e9ecef', textAlign: 'center' }}>
                <Box sx={{ display: 'flex', justifyContent: 'center' }}>
                  <Image src={current.imageItem.image} width={180} height={180} alt={current.imageItem.word} style={{ objectFit: 'contain' }} />
                </Box>
              </Paper>
            </Zoom>
