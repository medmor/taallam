...
const modes = { landmarks: { name: "معالم", emoji: "🗺️" }, map: { name: "خريطة بسيطة", emoji: "🧭" }, mixed: { name: "مختلط", emoji: "🎲" } };
const landmarks = [
  { city: "القاهرة", country: "مصر", icon: "🕌", label: "الأزهر" },
  { city: "الرياض", country: "السعودية", icon: "🕋", label: "الكعبة" },
  { city: "باريس", country: "فرنسا", icon: "🗼", label: "برج إيفل" },
  { city: "لندن", country: "بريطانيا", icon: "🎡", label: "عين لندن" },
];
const directions = ["الشمال", "الجنوب", "الشرق", "الغرب"]; const dirEmoji = { "الشمال":"⬆️","الجنوب":"⬇️","الشرق":"➡️","الغرب":"⬅️" };
function randFrom(arr,n){const c=[...arr];c.sort(()=>Math.random()-0.5);return n?c.slice(0,n):c[0];}

export default function CityCountryGame({ level="beginner", onComplete, onBack }={}){
  const router=useRouter(); const searchParams=useSearchParams();
  const [difficulty,setDifficulty]=useState(difficultyPresets[level]?level:"beginner");
  const [mode,setMode]=useState("landmarks"); const [started,setStarted]=useState(false);
  ...
  function makeQuestion(){ const actual=mode==='mixed'?randFrom(["landmarks","map"]):mode; if(actual==='landmarks'){ const item=randFrom(landmarks); const others=randFrom(landmarks.filter(l=>l.city!==item.city),3).map(x=>x.city); setQuestion({ type:actual, text:`${item.icon} في أي مدينة يوجد «${item.label}»؟`, correct:item.city, hint:`الدولة: ${item.country}` }); setOptions([item.city,...others].sort(()=>Math.random()-0.5)); } else { const dir=randFrom(directions); const wrong=randFrom(directions.filter(d=>d!==dir),3); setQuestion({ type:actual, text:`${dirEmoji[dir]} أي جهة؟`, correct:dir, hint:"اتجاهات بسيطة" }); setOptions([dir,...wrong].sort(()=>Math.random()-0.5)); }
  ...
  return (<Box sx={{maxWidth:900,mx:'auto',p:{xs:1.5,md:2}}}><Paper elevation={0} sx={{p:{xs:2,md:3},borderRadius:6,backgroundColor:'#fff',border:'3px solid #81c784'}}>
    <Box sx={{display:'flex',justifyContent:'space-between',alignItems:'center',mb:1}}>
      ...
      <Typography variant="h4" sx={{color:'#2e7d32',fontWeight:'bold'}}>مدينتي وبلدي</Typography>
    ...
    {!started? (<>
      ...
      <Box sx={{textAlign:'center',mb:1}}>
        <Typography variant="h6" sx={{color:'#1b5e20'}}>{question?.type==='landmarks'? 'اختر المدينة الصحيحة' : 'ما الجهة؟'}</Typography>
