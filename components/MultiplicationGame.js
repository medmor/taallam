...
function generateForLevel(level) {
  // Enhanced difficulty progression for multiplication
  let a,
    b,
    maxChoices = 4;
  ...
  while (wrongs.length < maxChoices - 1) {
    const delta = level === "expert" ? randInt(5, 20) : randInt(1, 10);
    ...
    // Alternative wrong answers using nearby multiplication results
    if (wrongs.length < maxChoices - 1) {
      const wrongMultiplication = (a + (Math.random() > 0.5 ? 1 : -1)) * b;
      if (wrongMultiplication > 0 && wrongMultiplication !== answer && !wrongs.includes(wrongMultiplication)) {
        wrongs.push(wrongMultiplication);
      }
    }
    
    if (wrongs.length < maxChoices - 1) {
      const wrongMultiplication2 = a * (b + (Math.random() > 0.5 ? 1 : -1));
      if (wrongMultiplication2 > 0 && wrongMultiplication2 !== answer && !wrongs.includes(wrongMultiplication2)) {
        wrongs.push(wrongMultiplication2);
      }
    }
  }
...
export default function MultiplicationGame({ level: initialLevel = "beginner", onComplete, onBack }) {
  ...
  const progressManager = useRef(new GameProgressionManager("multiplication-game"));
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            تحدي الضرب {difficultyLevels[level].icon}
          </Typography>
          <Chip
