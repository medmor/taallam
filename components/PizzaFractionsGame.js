...
function generateForLevel(level) {
  ...
  while (wrongs.length < maxChoices - 1) {
    const wrongNumerator = randInt(1, denominator - 1);
    const wrongDenominator = denominator; // Keep same denominator for easier comparison
    
    if (wrongNumerator !== numerator) {
      const fractionText = `${wrongNumerator}/${wrongDenominator}`;
      if (!wrongs.includes(fractionText)) {
        wrongs.push(fractionText);
      }
    }
    
    // Also add some with different denominators but similar values
    if (wrongs.length < maxChoices - 1) {
      const altDenominator = denominator === 4 ? 8 : denominator === 8 ? 4 : denominator;
      const altNumerator = Math.round((numerator / denominator) * altDenominator);
      if (altNumerator > 0 && altNumerator < altDenominator && altNumerator !== numerator) {
...
export default function PizzaFractionsGame({ level: initialLevel = "beginner", onComplete, onBack }) {
  ...
  // Function to render pizza visual
