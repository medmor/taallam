...
const modes = { facts: { name: "معلومات مبسطة", emoji: "📜" }, places: { name: "أماكن مشهورة", emoji: "🏺" }, mixed: { name: "مختلط", emoji: "🎲" } };
const egyptFacts = [
  { q: "أين توجد الأهرامات الشهيرة؟", correct: "مصر", wrong: ["اليونان", "إيطاليا", "الصين"], icon: "🏜️" },
  { q: "ما اسم نهر مصر؟", correct: "النيل", wrong: ["الأمازون", "الدانوب", "اليانغتسي"], icon: "🌊" },
  { q: "ما اسم الكتابة القديمة في مصر؟", correct: "الهيروغليفية", wrong: ["السنسكريتية", "اليونانية", "اللاتينية"], icon: "🔤" },
];
function randFrom(arr,n){const c=[...arr];c.sort(()=>Math.random()-0.5);return n?c.slice(0,n):c[0];}

export default function AncientCivilizationsGame({ level="beginner", onComplete, onBack }={}){
  const router=useRouter(); const searchParams=useSearchParams();
  const [difficulty,setDifficulty]=useState(difficultyPresets[level]?level:"beginner");
  const [mode,setMode]=useState("facts"); const [started,setStarted]=useState(false);
  ...
  function makeQuestion(){ const item=randFrom(egyptFacts); setQuestion({ type:'facts', text:`${item.icon} ${item.q}`, correct:item.correct, hint:"معلومة عن مصر القديمة" }); setOptions([item.correct,...randFrom([item.wrong],1)[0]].sort(()=>Math.random()-0.5)); setSelected(null); setShowResult(false);} 
  ...
  return (<Box sx={{maxWidth:900,mx:'auto',p:{xs:1.5,md:2}}}><Paper elevation={0} sx={{p:{xs:2,md:3},borderRadius:6,backgroundColor:'#fff',border:'3px solid #81c784'}}>
    <Box sx={{display:'flex',justifyContent:'space-between',alignItems:'center',mb:1}}>
      ...
      <Typography variant="h4" sx={{color:'#2e7d32',fontWeight:'bold'}}>الحضارات القديمة</Typography>
    ...
    {!started? (<>
      ...
      <Grid container spacing={2} sx={{mb:2}}>
        ...
        <Grid item xs={12} md={6}><FormControl fullWidth><InputLabel>الوضع</InputLabel><Select value={'facts'} label="الوضع" disabled><MenuItem value={'facts'}>📜 معلومات مبسطة</MenuItem></Select></FormControl></Grid>
      </Grid>
      ...
      <Box sx={{textAlign:'center',mb:1}}>
        <Typography variant="h6" sx={{color:'#1b5e20'}}>اختر الإجابة الصحيحة</Typography>
