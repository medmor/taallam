...
import { Box, Paper, Typography, Button, FormControl, InputLabel, Select, MenuItem, Stack } from '@mui/material';
...
export default function PuzzleAssembly({ snapThreshold = 40 }) {
  ...
  const draggingRef = useRef({ id: null, startX: 0, startY: 0, offsetX: 0, offsetY: 0 });
  ...
  useEffect(() => {
    ...
    const colors = ['#ef4444', '#f59e0b', '#10b981', '#3b82f6', '#6366f1', '#e11d48', '#059669', '#fbbf24', '#0ea5e9', '#a21caf', '#f43f5e', '#22d3ee', '#f87171', '#65a30d', '#facc15', '#2563eb'];
    let t = [];
    ...
    try {
      ...
      const valid = allItems.filter(it => it && it.image && !it.image.includes('globe.svg'));
      if (valid.length > 0) {
        const pick = valid[Math.floor(Math.random() * valid.length)];
        chosenImage = pick.image;
      } else if (allItems.length > 0) {
        chosenImage = allItems[Math.floor(Math.random() * allItems.length)].image;
      }
    } catch (e) { chosenImage = null; }
    ...
    const resize = () => {
      try {
        const w = boardRef.current?.parentElement?.clientWidth || window.innerWidth;
  ...
  const onPointerDown = (e, id) => {
    ...
    draggingRef.current = { id, startX: pt.bx, startY: pt.by, offsetX: pt.bx - p.x, offsetY: pt.by - p.y };
  ...
  if (!startedAt) setStartedAt(Date.now());
  setPieces(prev => prev.map(p => p.id === id ? { ...p, x: pt.bx - offsetX, y: pt.by - offsetY } : p));
  };
  ...
  const onPointerUp = (e) => {
    ...
    const p = pieces.find(x => x.id === id);
    if (!p) { draggingRef.current = { id: null }; return; }
    // check snap
    ...
    if (dist <= snapThreshold) {
      // snap into place
  setPieces(prev => prev.map(pp => pp.id === id ? { ...pp, x: pp.targetX, y: pp.targetY, snapped: true } : pp));
  ...
  const reset = () => {
    ...
    setPieces(prev => prev.map((p, i) => ({ ...p, x: Math.random() * (boardW - p.w - 8) + 4, y: Math.random() * (boardH - p.h - 8) + 4, snapped: false })));
  };
  ...
  const changeImage = () => {
    ...
    setCompletedAt(null);
    setStartedAt(null);
    setPieces(prev => prev.map(p => ({ ...p, x: Math.random() * (boardW - p.w - 8) + 4, y: Math.random() * (boardH - p.h - 8) + 4, snapped: false })));
  };
  ...
  return (
    <Box sx={{ width: '100%', maxWidth: 760, mx: 'auto', position: 'relative' }}>
      <Typography variant="h5" sx={{ mb: 1, textAlign: 'center' }}>Puzzle Assembly — تركيب الأشكال</Typography>
      <Typography variant="body2" sx={{ mb: 2, textAlign: 'center' }}>اسحب القطع إلى الداخل حتى تتطابق مع الشكل الظل. ستلتصق القطع عندما تكون قريبة بما فيه الكفاية.</Typography>

      <Paper sx={{ p: 2, mb: 2 }}>
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', justifyContent: 'center', flexWrap: 'wrap' }}>
          <Typography>الوقت: {formatMs(startedAt && completedAt ? completedAt - startedAt : (startedAt ? Date.now() - startedAt : 0))}</Typography>
          <Typography>الحالة: {completedAt ? 'مكتمل' : 'قيد اللعب'}</Typography>
          <Button variant="contained" color="primary" onClick={reset}>إعادة ضبط</Button>
          <Button variant="outlined" sx={{ ml: 1 }} onClick={changeImage}>تغيير الصورة</Button>
      ...
      <div
        ...
        style={{ width: boardW * scale, height: boardH * scale, margin: '0 auto', touchAction: 'none' }}>
        <svg width={boardW * scale} height={boardH * scale} viewBox={`0 0 ${boardW} ${boardH}`} style={{ display: 'block' }}>
          {/* silhouette */}
          <rect x={0} y={0} width={boardW} height={boardH} fill="#fff" stroke="#ccc" strokeWidth={2} />
          {/* ghost targets */}
          {pieces.map(p => (
            <rect key={`t-${p.id}`} x={p.targetX} y={p.targetY} width={p.w} height={p.h} fill="none" stroke="#999" strokeDasharray="4 4" />
          ))}

          {/* pieces (render on top) */}
          <defs>
            {pieces.map(p => (
              <clipPath id={`clip-${p.id}`} key={`clip-${p.id}`} clipPathUnits="userSpaceOnUse">
          ...
          {(() => {
            const sortedPieces = [...pieces].sort((a, b) => a.snapped - b.snapped);
            return sortedPieces.map(p => (
              ...
              >
                {imageSrc ? (
                  <g onPointerDown={(e) => onPointerDown(e, p.id)} style={{ cursor: p.snapped ? 'default' : 'grab' }}>
                    {(() => {
                      // Ensure precise alignment of tiles with the image
                      const imgW = 800, imgH = 533;
                      return (
                        <image
                          href={imageSrc}
                          x={-p.targetX}
                    ...
                    <rect x={0} y={0} width={p.w} height={p.h} rx={8} ry={8} fill="none" stroke={p.snapped ? '#444' : '#222'} strokeWidth={p.snapped ? 3 : 1} />
                  </g>
                ) : (
                  <rect
                    x={0}
      ...
      {showWin && (
        <WinOverlay boardPixel={Math.min(boardW, 320)} moves={0} errors={0} onPlayAgain={() => { reset(); }} />
      )}
    </Box>
  );
}
