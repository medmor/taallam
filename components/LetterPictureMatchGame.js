...
import {
  Box, Button, Chip, Fade, FormControl, Grid, InputLabel, LinearProgress, MenuItem, Paper, Select, Typography, Zoom
} from '@mui/material';
...
const LEVEL_CONFIG = {
  beginner: { rounds: 10, choices: 3 },
  intermediate: { rounds: 15, choices: 4 },
  advanced: { rounds: 20, choices: 4 },
};

export default function LetterPictureMatchGame({ level: initialLevel = 'beginner', onComplete, onBack }) {
  ...
  const progressManager = useRef(new GameProgressionManager('letter-picture'));
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mt: 4, position: 'relative' }}>
      ...
      <Paper elevation={8} sx={{ p: 4, borderRadius: 4, minWidth: 380, maxWidth: 700, border: `3px solid ${difficultyLevels[level].color}`, boxShadow: `0 8px 32px ${difficultyLevels[level].color}40`, background: 'linear-gradient(145deg, #ffffff, #f8f9fa)', position: 'relative', overflow: 'hidden' }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          ...
          <Typography variant="h4" align="center" sx={{ fontWeight: 'bold', color: difficultyLevels[level].color, flexGrow: 1 }}>التعرف على الحروف بالصور</Typography>
          ...
          <FormControl fullWidth size="small">
            ...
            <Select value={level} label="مستوى الصعوبة" onChange={(e) => setLevel(e.target.value)} disabled={showFeedback}>
              {Object.entries(difficultyLevels).map(([key, config]) => (
                <MenuItem key={key} value={key}>{config.icon} {config.name}</MenuItem>
              ))}
            </Select>
          </FormControl>
        </Box>
          ...
          <>
            <Zoom in={!showFeedback} timeout={500}>
              <Paper elevation={4} sx={{ p: 3, mb: 3, borderRadius: 3, backgroundColor: '#f8f9fa', border: '2px solid #e9ecef', textAlign: 'center' }}>
                <Typography variant="h6" sx={{ mb: 2, color: '#2c3e50' }}>اختر الصورة التي تبدأ بحرف</Typography>
                ...
                {current.options.map((opt, idx) => (
                  <Grid item key={opt.id}>
                    <Zoom in timeout={300} style={{ transitionDelay: `${idx * 100}ms` }}>
                      <Button variant="outlined" onClick={() => pick(opt)} sx={{ p: 1.5, borderRadius: 3, border: '2px solid #e0e0e0', backgroundColor: 'white', '&:hover': { border: `2px solid ${difficultyLevels[level].color}`, boxShadow: `0 8px 16px ${difficultyLevels[level].color}40` } }}>
                        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                          <Image src={opt.image} width={100} height={100} alt={opt.word} style={{ objectFit: 'contain' }} />
                          <Typography variant="subtitle1" sx={{ mt: 1, fontWeight: 'bold', color: '#333' }}>{opt.word}</Typography>
                        </Box>
                      </Button>
                    </Zoom>
                  </Grid>
        ...
        <Box sx={{ mt: 3, textAlign: 'center' }}>
          ...
          {finalTime !== null && (
            <Typography sx={{ mt: 1, fontSize: 16, color: '#00838f', textAlign: 'center' }}>
              الوقت المستغرق: {Math.floor(finalTime / 60).toString().padStart(2, '0')}:{(finalTime % 60).toString().padStart(2, '0')}
            </Typography>
          )}
        </Box>
      </Paper>

      {/* No WinOverlay; parent handles navigation on onComplete */}
    </Box>
  );
}
