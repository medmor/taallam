...
const DecimalRace = () => {
  ...
  const generateIdentifyQuestion = () => {
    ...
    const questionTypes = ["digit-value", "place-name", "digit-position"];
    ...
    if (questionType === "digit-value") {
      ...
      wrongOptions.add(
        (
          targetDigit * placeValues[(digitIndex + 1) % placeValues.length]
        ).toFixed(3)
      );
      wrongOptions.add(
        (
          targetDigit * placeValues[(digitIndex + 2) % placeValues.length]
        ).toFixed(3)
      );
      wrongOptions.add(targetDigit.toString());

      setOptions(
        [correctAnswer, ...Array.from(wrongOptions)].sort(
          () => Math.random() - 0.5
        )
      );
      ...
      questionText = `في العدد ${decimal}، أي رقم في خانة ${
        ["العُشر", "المئة", "الألف"][digitIndex]
      }؟`;
      ...
      const allOptions = [correctAnswer, ...Array.from(wrongOptions)].sort(
        () => Math.random() - 0.5
      );
  ...
  const generateCompareQuestion = () => {
    ...
    setOptions(
      [correctAnswer, wrongAnswer, equalAnswer].sort(() => Math.random() - 0.5)
    );
  ...
  const generatePlaceValueQuestion = () => {
    ...
    const targetValue =
      placeValues[Math.floor(Math.random() * placeValues.length)];
  ...
  return (
    ...
    >
        ...
        <>
          ...
          >
            {options.map((option, index) => {
              // Format decimals: max 3 places, remove trailing zeros
              let displayOption = option;
              if (typeof option === 'string' && /^-?\d*\.?\d+$/.test(option)) {
                const num = Number(option);
                if (!isNaN(num)) {
                  displayOption = parseFloat(num.toFixed(3)).toString();
                }
              }
              return (
                <Button
      ...
      <Box sx={{ mt: 3, p: 2, backgroundColor: "#e3f2fd", borderRadius: 2 }}>
        ...
        <Typography variant="body2" sx={{ mb: 1 }}>
          🔢 <strong>خانة المئة:</strong> الرقم الثاني بعد الفاصلة العشرية
          (0.01)
        </Typography>
        <Typography variant="body2" sx={{ mb: 1 }}>
          🔢 <strong>خانة الألف:</strong> الرقم الثالث بعد الفاصلة العشرية
          (0.001)
        </Typography>
        <Typography variant="body2" sx={{ mb: 1 }}>
