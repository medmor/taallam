...
export class Level {
    ...
    this.height = levelDef.height || defaultHeight;
        ...
        objs.forEach(o => {
            ...
            } else if (o.type === 'triangle' || o.type === 'pin') {
                // create an isosceles triangle with given width (base) and height
                const w = o.w || 32;
                const h = o.h || 32;
                // vertices relative to center (clockwise)
                const verts = [
                    { x: -w / 2, y: h / 2 },
                    { x: w / 2, y: h / 2 },
                    { x: 0, y: -h / 2 }
                ];
                // Matter.Bodies.fromVertices expects an array of vertex sets; provide verts directly
                let body = null;
                try {
                    const created = Matter.Bodies.fromVertices(o.x, o.y, verts, { isStatic: !!o.isStatic });
                    body = created;
                } catch (e) {
                    // fallback: create a small rectangle if fromVertices isn't available
                    body = Matter.Bodies.rectangle(o.x, o.y, w, h, { isStatic: !!o.isStatic });
                }
                if (body) {
                    // mark as obstacle so player can detect it
                    try { body.isObstacle = true; } catch (e) { body.label = (body.label || '') + ' obstacle'; }
                    this.bodies.push({ name: o.name || '', body, def: o });
                }

                const g = new PIXI.Graphics();
                let colorVal = resolveColor(o.color || '0xff4444');
