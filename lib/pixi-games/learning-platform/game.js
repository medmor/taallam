...
export default class Game {
    ...
    this.app.stage.addChild(this.debugHud);
        ...
        // input handlers
        this._onKeyDown = (e) => {
            // toggle debug overlay with 'd' (case-insensitive)
            try {
                if (typeof e.key === 'string' && e.key.toLowerCase() === 'd') {
                    this.toggleDebug();
                    return;
                }
            } catch (ex) { }
    ...
    toggleDebug(visible) {
        if (typeof visible === 'boolean') this.debugVisible = visible;
        else this.debugVisible = !this.debugVisible;
        try {
            if (this.debugG) this.debugG.visible = !!this.debugVisible;
            if (this.debugHud) this.debugHud.visible = !!this.debugVisible;
            if (this.debugBg) this.debugBg.visible = !!this.debugVisible;
        } catch (e) { }
    }
    ...
    loop() {
        ...
        if (this.player) {
            ...
            // offset camera slightly above the player (10% of screen height)
            ...
            try {
                ...
                const debugText = `groundY: ${Math.round(groundMiddleY)}  topY(world): ${Math.round(cameraTopWorldY)}  pivotY: ${Math.round(pivotY)}  playerY: ${playerY}`;
                if (this.debugHud) this.debugHud.text = debugText;
