...
export class Player {
                ...
                if (a === this.body || b === this.body) {
                    ...
                    while (current && !found) {
                        ...
                        if (current.isObstacle || (label && label.includes('obstacle'))) {
                            found = true;
                            if (typeof current.damage === 'number' && !isNaN(current.damage)) damage = current.damage;
                        }
    ...
    _onHitObstacle(obstacleBody, damage = 1) {
        ...
        const dmg = (typeof damage === 'number' && !isNaN(damage) && damage > 0) ? Math.floor(Math.abs(damage)) : 1;
        // reduce health
