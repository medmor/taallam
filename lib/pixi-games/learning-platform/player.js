...
export class Player {
    ...
    jump(jumpPower, groundY) {
        ...
        if (grounded) {
            ...
            if (typeof playSfx === 'function') {
                try { playSfx('jump'); } catch (e) { console.log('jump sfx error', e);}
            }
        }
    }
        ...
        this._collisionStart = (event) => {
            for (const pair of event.pairs) {
                ...
                if (a === this.body || b === this.body) {
                    ...
                    // obstacle damage check: some bodies are marked as obstacles
                    try {
                        const isObstacle = !!other.isObstacle || (other.label && String(other.label).toLowerCase().includes('obstacle'));
                        if (isObstacle) {
                            this._onHitObstacle(other);
                        }
                    } catch (e) { }
                }
            }
    ...
    _onHitObstacle(obstacleBody) {
        ...
        try { this.gfx.tint = 0xffcccc; } catch (e) { }
        setTimeout(() => { try { this._invulnerable = false; this.gfx.tint = 0xffffff; } catch (e) { } }, this._invulnMs);
        ...
        try {
            ...
            // small scale pulse
            try { this.gfx.scale.set(1.3); setTimeout(() => { try { this.gfx.scale.set(1); } catch (e) { } }, 150); } catch (e) { }
        } catch (e) { }
    ...
    dieAndReset() {
        ...
        try { if (typeof playSfx === 'function') playSfx('death'); } catch (e) { }
        // teleport to spawn
        try { this.Matter.Body.setPosition(this.body, { x: this._spawn.x, y: this._spawn.y }); this.Matter.Body.setVelocity(this.body, { x: 0, y: 0 }); } catch (e) { }
        // reset health
        this.health = this.maxHealth;
        this._invulnerable = true;
        setTimeout(() => { this._invulnerable = false; }, 500);
        // small reset animation pulse
        try { this.gfx.scale.set(1.2); setTimeout(() => { try { this.gfx.scale.set(1); } catch (e) { } }, 150); } catch (e) { }
    }
    ...
    updateCamera(worldContainer, levelWidth, levelHeight, screenWidth, screenHeight, smooth = 0.12, opts = {}) {
        ...
        else if (opts && typeof opts.offsetFactor === 'number') offset = (opts.offsetFactor * screenHeight);
