...
export default class UIManager {
    ...
    this.container.addChild(this.healthBg, this.healthFg, this.timerText, this.messageText);
        ...
        this.muteIcon = new PIXI.Text( {text: '🔊', style: { fontFamily: 'monospace', fontSize: 16 }}); this.muteIcon.x = 6; this.muteIcon.y = 4;
        this.muteBtn.addChild(mbg, this.muteIcon);
        this.muteBtn.x = this._barX + this._barW + 12; this.muteBtn.y = this._barY - 4;
        this.muteBtn.interactive = true; this.muteBtn.buttonMode = true;
        this.muteBtn.on('pointerdown', () => {
            this._isMuted = !this._isMuted; try { this.muteIcon.text = this._isMuted ? '🔇' : '🔊'; } catch (e) {}
            if (typeof this.onMuteToggle === 'function') this.onMuteToggle(this._isMuted);
        });
        ...
        this.pauseIcon = new PIXI.Text({text: '⏸', style: { fontFamily: 'monospace', fontSize: 16 }}); this.pauseIcon.x = 6; this.pauseIcon.y = 4;
        this.pauseBtn.addChild(pbg, this.pauseIcon);
        this.pauseBtn.x = this.muteBtn.x + this.btnSize + 8; this.pauseBtn.y = this.muteBtn.y;
        this.pauseBtn.interactive = true; this.pauseBtn.buttonMode = true;
        this.pauseBtn.on('pointerdown', () => {
            this._isPaused = !this._isPaused; try { this.pauseIcon.text = this._isPaused ? '▶' : '⏸'; } catch (e) {}
            if (typeof this.onPauseToggle === 'function') this.onPauseToggle(this._isPaused);
        });
