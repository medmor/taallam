...
export default class UIManager {
  constructor(PIXI, stage, opts = {}) {
    ...
    this.healthBg.rect(0, 0, this._barW, this._barH);
    this.healthBg.fill({ color: 0x222222, alpha: 0.8 });
    this.healthBg.x = this._barX;
    this.healthBg.y = this._barY;

    this.healthFg = new PIXI.Graphics();
    this.healthFg.rect(0, 0, this._barW, this._barH);
    this.healthFg.fill({ color: 0xff4444 });
    this.healthFg.x = this._barX;
    this.healthFg.y = this._barY;
    ...
    this.timerText.x = this.width - 80;
    this.timerText.y = 12;
    ...
    this.onMuteToggle = null; // callback(fn(isMuted))
    this.onPauseToggle = null; // callback(fn(isPaused))
    this._isMuted = false;
    ...
    mbg.rect(0, 0, this.btnSize, this.btnSize);
    mbg.fill({ color: 0x000000, alpha: 0.4 });
    ...
    this.muteIcon.x = 6;
    this.muteIcon.y = 4;
    this.muteBtn.addChild(mbg, this.muteIcon);
    this.muteBtn.x = this._barX + this._barW + 12;
    this.muteBtn.y = this._barY - 4;
    this.muteBtn.interactive = true;
    this.muteBtn.buttonMode = true;
    this.muteBtn.on("pointerdown", () => {
      this._isMuted = !this._isMuted;
      try {
        this.muteIcon.text = this._isMuted ? "🔇" : "🔊";
      } catch (e) {}
      if (typeof this.onMuteToggle === "function")
        this.onMuteToggle(this._isMuted);
    });
    this.container.addChild(this.muteBtn);

    // pause button
    this.pauseBtn = new PIXI.Container();
    const pbg = new PIXI.Graphics();
    pbg.rect(0, 0, this.btnSize, this.btnSize);
    pbg.fill({ color: 0x000000, alpha: 0.4 });
    this.pauseIcon = new PIXI.Text({
      text: "⏸",
      style: { fontFamily: "monospace", fontSize: 16 },
    });
    this.pauseIcon.x = 6;
    this.pauseIcon.y = 4;
    this.pauseBtn.addChild(pbg, this.pauseIcon);
    this.pauseBtn.x = this.muteBtn.x + this.btnSize + 8;
    this.pauseBtn.y = this.muteBtn.y;
    this.pauseBtn.interactive = true;
    this.pauseBtn.buttonMode = true;
    this.pauseBtn.on("pointerdown", () => {
      this._isPaused = !this._isPaused;
      try {
        this.pauseIcon.text = this._isPaused ? "▶" : "⏸";
      } catch (e) {}
  ...
  setHealth(current, max) {
    ...
    try {
      ...
      this.healthFg.clear();
      this.healthFg.rect(0, 0, w, this._barH);
      this.healthFg.fill({ color });
    } catch (e) {}
  ...
  update(deltaMs) {
    ...
    try {
      this.timerText.text = `${String(m).padStart(2, "0")}:${String(s).padStart(
        2,
        "0"
      )}`;
    } catch (e) {}
  ...
  destroy() {
    ...
    try {
      this.container.removeChild(this.healthBg);
      this.container.removeChild(this.healthFg);
      this.container.removeChild(this.timerText);
      this.container.removeChild(this.messageText);
    } catch (e) {}
    try {
      this.healthBg.destroy && this.healthBg.destroy({ children: true });
    } catch (e) {}
    try {
      this.healthFg.destroy && this.healthFg.destroy({ children: true });
    } catch (e) {}
    try {
      this.timerText.destroy && this.timerText.destroy({ children: true });
    } catch (e) {}
    try {
      this.messageText.destroy && this.messageText.destroy({ children: true });
    } catch (e) {}
    try {
      this.container.destroy && this.container.destroy({ children: true });
    } catch (e) {}
  }
}
