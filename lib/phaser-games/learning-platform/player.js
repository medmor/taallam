...
export default class Player {
  constructor(scene, x = 100, y = 100, options = {}) {
    ...
    if (scene.textures.exists(texKey)) {
      this.sprite = scene.physics.add.sprite(x, y, texKey).setDisplaySize(diameter, diameter);
    } else {
      // still create a physics sprite even if texture is missing
      this.sprite = scene.physics.add.sprite(x, y, null).setDisplaySize(diameter, diameter).setTint(colors.player);
    }
    // configure body as a circle if supported, otherwise set the size to the diameter
    if (this.sprite.body && typeof this.sprite.body.setCircle === 'function') {
      this.sprite.body.setCircle(radius);
    } else if (this.sprite.body && typeof this.sprite.body.setSize === 'function') {
      this.sprite.body.setSize(diameter, diameter);
    }
