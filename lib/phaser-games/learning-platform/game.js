...
export default class Game {
  ...
  async create(scene) {
    ...
    for (let y = 0; y < worldH; y += 2) {
      const t = y / worldH;
      const r = Math.round(
        (1 - t) * ((topColor >> 16) & 0xff) + t * ((bottomColor >> 16) & 0xff)
      );
      const g = Math.round(
        (1 - t) * ((topColor >> 8) & 0xff) + t * ((bottomColor >> 8) & 0xff)
      );
      const b = Math.round(
        (1 - t) * (topColor & 0xff) + t * (bottomColor & 0xff)
      );
    ...
    if (lvl && lvl.objects) {
      for (const obj of lvl.objects) {
        if (obj.type === "rect") {
          ...
          const { default: SpikesBase } = await import(
            "./enemies/spikes/SpikesBase.js"
          );
    ...
    this._uiTogglePauseHandler = () => {
      try {
        ...
        if (scene.physics && scene.physics.world) {
          try {
            if (this._paused && typeof scene.physics.world.pause === "function")
              scene.physics.world.pause();
            else if (
              !this._paused &&
              typeof scene.physics.world.resume === "function"
            )
              scene.physics.world.resume();
          } catch (e) {}
        }
        try {
          scene.sys.events.emit("gamePaused", this._paused);
        } catch (e) {}
      } catch (e) {}
    };
    this._uiToggleSoundHandler = () => {
      try {
        ...
        try {
          scene.sys.events.emit("soundMutedChanged", !!newMuted);
        } catch (e) {}
      } catch (e) {}
    };
    try {
      scene.sys &&
        scene.sys.events &&
        scene.sys.events.on &&
        scene.sys.events.on("ui:togglePause", this._uiTogglePauseHandler);
      scene.sys &&
        scene.sys.events &&
        scene.sys.events.on &&
        scene.sys.events.on("ui:toggleSound", this._uiToggleSoundHandler);
      // fullscreen toggle
      ...
      // listen to scale manager enter/leave to notify UI
      try {
        if (scene.scale && scene.scale.on) {
          ...
          scene.scale.on("leavefullscreen", () => {
            try {
              // Resize back to original container size
          ...
          // emit resize when the scale manager resizes the canvas
    ...
    try {
      if (
        scene.sound &&
        scene.cache.audio.exists &&
        scene.cache.audio.exists("theme")
      ) {
    ...
    scene.physics.add.collider(this.playerObj.sprite, spikes, (p, s) => {
      try {
        ...
        try {
          this.playerObj &&
            typeof this.playerObj.takeDamage === "function" &&
  ...
  update(scene, time, delta) {
    ...
    if (scene.cameras && scene.cameras.main && this.playerObj.sprite) {
      ...
      scene.cameras.main.scrollY =
        this.playerObj.sprite.y + camYOffset - scene.cameras.main.height / 2;
    }
    ...
    if (Math.abs(vx) > this.playerObj.sprite.maxXvelocity) {
      this.playerObj.sprite.setVelocityX(
        Math.sign(vx) * this.playerObj.sprite.maxXvelocity
      );
    }
    if (Math.abs(vy) > this.playerObj.sprite.maxYvelocity) {
      this.playerObj.sprite.setVelocityY(
        Math.sign(vy) * this.playerObj.sprite.maxYvelocity
      );
    }
  }
