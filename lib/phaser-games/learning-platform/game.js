...
export default class Game {
  ...
  async create(scene) {
    ...
    if (!PlayerClass)
      throw new Error("Player module failed to load: ./player.js");
    if (!PlatformBase)
      throw new Error(
        "PlatformBase module failed to load: ./platforms/platformBase.js"
      );
    if (!UIClass) throw new Error("UI module failed to load: ./ui.js");
    ...
    scene.player = this.playerObj.sprite;
    ...
    scene.physics.add.overlap(scene.player, scene.spikes, (p, s) => {
      // simple hit -> reset player
      p.setTint(0xff0000);
    ...
    scene.physics.add.overlap(scene.player, scene.collectibles, (p, c) => {
      try {
        ...
        if (
          this._collected >=
          (this._totalCollectibles || scene.collectibles.getLength())
        ) {
          this._emit("win");
        }
      } catch (e) {}
    ...
    const list = scene.collectibles.getChildren().slice();
    const ordered = list
  ...
  update(scene, time, delta) {
    ...
    if (this.playerObj && typeof this.playerObj.update === "function") {
      try {
    ...
    if (scene.cameras && scene.cameras.main && scene.player) {
      ...
      scene.cameras.main.scrollY = scene.player.y + camYOffset - scene.cameras.main.height / 2;
    }

    // Clamp velocities (ensure max speeds)
    const vx = scene.player.body.velocity.x;
    const vy = scene.player.body.velocity.y;
    if (Math.abs(vx) > scene.player.maxXvelocity) {
      scene.player.setVelocityX(Math.sign(vx) * scene.player.maxXvelocity);
    }
    if (Math.abs(vy) > scene.player.maxYvelocity) {
      scene.player.setVelocityY(Math.sign(vy) * scene.player.maxYvelocity);
    }

    // Simple animation: scale/pulse based on state (reads properties on sprite)
    if (!scene.player._animScale) scene.player._animScale = 1;
    if (!scene.player._targetScale) scene.player._targetScale = 1;
    const blockedDown = !!(
      scene.player.body &&
      scene.player.body.blocked &&
      scene.player.body.blocked.down
    );
    if (!blockedDown) {
      scene.player._targetScale = 0.9;
    } else if (scene._lastMove !== 0) {
      scene.player._targetScale = 1.05;
    } else {
      scene.player._targetScale = 1;
    }
    scene.player._animScale +=
      (scene.player._targetScale - scene.player._animScale) * 0.15;
    scene.player.setScale(scene.player._animScale, 1.0);
  }
