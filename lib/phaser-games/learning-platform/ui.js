export default class UIManager {
  constructor(scene) {
    ...
    try {
      ...
      this.scene &&
        ...
        this.scene.sys.events &&
        this.scene.sys.events.on &&
        this.scene.sys.events.on("resize", this._onResize, this);
    } catch (e) {}
  }
  ...
  _create() {
    ...
    try {
        ...
        const gap = 8;
        const camWidth = camW;
        // anchor buttons to right margin
        const rightMargin = 16;
        const soundX = camWidth - rightMargin - btnSize / 2;
        const fullscreenX = soundX - (btnSize + gap);
        const pauseX = fullscreenX - (btnSize + gap);
        const btnY = containerCenterY;
        // compute container left so it doesn't overlap buttons
        const containerRight = pauseX - gap;
        const containerLeft = Math.max(8, containerRight - w);
        const adjustedCenterX = containerLeft + w / 2;
        // reposition container and bar accordingly (use consistent padding)
        this.healthContainer.setPosition(adjustedCenterX, containerCenterY);
      ...
      this.soundIcon = scene.add
        ...
        .setDepth(1101)
        .setOrigin(0.5);
        // fullscreen button (use simple ASCII label to avoid emoji/font rendering issues)
        this.fullscreenBtn = scene.add
          .rectangle(fullscreenX, btnY, btnSize, btnSize, 0x444444)
          .setScrollFactor(0)
          .setDepth(1100)
          .setOrigin(0.5)
          .setInteractive({ useHandCursor: true });
        this.fullscreenIcon = scene.add
          .text(fullscreenX, btnY, "FS", { fontSize: "12px", color: "#fff" })
          .setScrollFactor(0)
          .setDepth(1101)
          .setOrigin(0.5);
      ...
      this.fullscreenBtn.on("pointerdown", () => {
        try {
          this.scene &&
            ...
            this.scene.sys.events &&
            this.scene.sys.events.emit &&
            this.scene.sys.events.emit("ui:toggleFullscreen");
        } catch (e) {}
      });
    } catch (e) {}
  }
  ...
  _onResize() {
    ...
    try {
      ...
      const containerCenterY = this.healthContainer ? this.healthContainer.y : 16 + (this.healthContainer ? this.healthContainer.height/2 : 9);
      // reposition
      this.healthContainer && this.healthContainer.setPosition(containerCenterX, containerCenterY);
      this.healthBar && this.healthBar.setPosition(containerLeft + 2, containerCenterY);
      ...
      this.fullscreenBtn && this.fullscreenBtn.setPosition(fullscreenX, containerCenterY);
      this.fullscreenIcon && this.fullscreenIcon.setPosition(fullscreenX, containerCenterY);
  ...
  destroy() {
    ...
    try {
      this.scene &&
        ...
        this.scene.sys.events &&
        this.scene.sys.events.off &&
        this.scene.sys.events.off("resize", this._onResize, this);
    } catch (e) {}
  }
}
