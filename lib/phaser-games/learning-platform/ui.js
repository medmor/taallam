...
export default class UIManager {
  constructor(scene) {
    ...
    try {
      ...
      this.scene?.sys?.events?.on("inventoryItemAdded", this._onInventoryItemAdded, this);
    } catch (e) {}
  }
...
_create() {
  ...
  // --- Buttons ---
  this.pauseBtn = createButton(pauseX, btnY, "⏸", "ui:togglePause");
  this.soundBtn = createButton(soundX, btnY, "🔊", "ui:toggleSound");
  this.fullscreenBtn = createButton(fullscreenX, btnY, "FS", "ui:toggleFullscreen", "12px");
  ...
  this.coinIcon = new Coin(scene, coinIconX, coinIconY, { radius: 10, color: 0xffd700 });
  this.coinIcon.sprite.setScrollFactor(0).setDepth(1000);

  // Place coin text info under the coin icon
  this.coinText = scene.add
    .text(coinIconX, coinIconY + 16, "0", { fontSize: "16px", color: "#fff" })
    .setScrollFactor(0)
    .setDepth(1000)
    .setOrigin(0.5, 0);
}
  ...
  _onInventoryItemAdded(item) {
    console.log(item)
    if (item.type === "coin") {
      this._updateCoinCount();
    }
  }

  _updateCoinCount() {
    const coins = this.scene.player.inventory.getItems().filter((item) => item.type === "coin");
    const totalCoins = coins.reduce((sum, coin) => sum + (coin.value || 1), 0);
